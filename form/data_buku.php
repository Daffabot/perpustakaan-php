		...
		<table width="100%" class="table table-striped table-bordered table-hover">
		  ...
		  <?php
			...
			while($tampil = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
				$sinopsis = $tampil['sinopsis'] ? substr($tampil['sinopsis'], 0, 250) : 'Sinopsis tidak tersedia';
				?>
				<tr>
					<td><?php echo $no; ?></td>
					...
					<?php } else { ?>
						...
						<td>
							<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='hapus_buku.php?id=<?php echo $tampil['id_t_buku']; ?>' }" 
