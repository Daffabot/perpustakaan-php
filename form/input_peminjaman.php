...
$error = "";
$id_pj = isset($_GET['id']) ? mysqli_real_escape_string($db, $_GET['id']) : 0;
...
if ($id_pj == 0) {
    ...
    // Jika tidak ada data, inisialisasi array default
    if (!$tampil_data) {
        $tampil_data = [
            'no_peminjaman' => '',
            'staff' => '',
...
if (isset($_POST['btnsubmit'])) {
    ...
    if (!$id_ag) {
        ...
        if ($id_pj == 0) {
            $sql_2 = "SELECT IFNULL(MAX(no_peminjaman), 'PJ-1000000') AS nopj FROM t_peminjaman";
            $result_2 = mysqli_query($db, $sql_2);
            $row_2 = mysqli_fetch_array($result_2, MYSQLI_ASSOC);
            $temp = $row_2['nopj'];

            $temp_pj = intval(substr($temp, 3)) + 1;
            $no_peminjaman = "PJ-" . str_pad($temp_pj, 7, '0', STR_PAD_LEFT);
